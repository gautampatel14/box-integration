...
logger = log.Logs().get_logger("box_integration_for_splunk_rh_oauth2_token")
...
class box_integration_for_splunk_rh_oauth2_token(admin.MConfigHandler):
    ...
    def handleEdit(self, confInfo):
        try:
            ...
            resp, content = http.request(
                url, method=method, headers=headers, body=urlencode(payload)
            )
            ...
            logger.exception("Error occurred while getting accesstoken using auth code")
            raise exc()
    ...
    def getProxyDetails(self):
        ...
        cfm = conf_manager.ConfManager(
            self.getSessionKey(),
            "box_integration_for_splunk",
            realm="__REST_CREDENTIAL__#box_integration_for_splunk#configs/conf-box_integration_for_splunk_settings",
        )
        # Get Conf object of apps settings
        conf = cfm.get_conf("box_integration_for_splunk_settings")
...
if __name__ == "__main__":
    admin.init(box_integration_for_splunk_rh_oauth2_token, admin.CONTEXT_APP_AND_USER)
